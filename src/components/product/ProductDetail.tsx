...
const ProductDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (product && product.price_slabs && selectedSlab) {
      ...
      const newSlab = product.price_slabs.find(
        (slab) =>
  ...
  const getWhatsAppLink = () => {
    if (!product) return "";
    const message = `Hi! I want to order ${quantity} bags of ${product.name} (${
      product.weight
    }). Total: ₹${(selectedSlab?.price_per_bag || 0) * quantity}`;
